...
public class DashboardPage extends JFrame {
    ...
    private JPanel buildNameRow() {
        ...
        JButton postItemBtn = createRoundedButton("📋  Post Item", PRIMARY, WHITE);
        postItemBtn.addActionListener(e -> new PostItemPage(studentName));

        JButton historyBtn = createRoundedButton("🕐  History", PRIMARY, WHITE);

        btnPanel.add(postItemBtn);
    ...
    private JScrollPane buildItemsPanel() {
        ...
        for (String[] item : items) {
            ...
            card.setPreferredSize(new Dimension(230, 360));
            grid.add(card);
        }

        JPanel centeringWrapper = new JPanel(new GridBagLayout());
        centeringWrapper.setBackground(BG);
        centeringWrapper.add(grid);
    ...
    private JPanel createItemCard(String name, String owner, String status, String tags) {
        ...
        imgBox.setBorder(new MatteBorder(0, 0, 1, 0, BORDER_LIGHT));
        ...
        // 2. TEXT SECTION (Ensuring strict Left Alignment)
        JPanel textSection = new JPanel();
        textSection.setLayout(new BoxLayout(textSection, BoxLayout.Y_AXIS));
        textSection.setBackground(WHITE);
        textSection.setBorder(new EmptyBorder(12, 15, 0, 15));
        // CRITICAL: This tells the BoxLayout to align its children to the left
        ...
        nameLbl.setAlignmentX(Component.LEFT_ALIGNMENT); // Left align
        textSection.add(nameLbl);
        textSection.add(Box.createVerticalStrut(4));

        // Owner
        JLabel ownerLbl = new JLabel("Owner: " + owner);
        ownerLbl.setFont(new Font("SansSerif", Font.PLAIN, 13));
        ownerLbl.setForeground(TEXT_MUTED);
        ownerLbl.setAlignmentX(Component.LEFT_ALIGNMENT); // Left align
        textSection.add(ownerLbl);
        textSection.add(Box.createVerticalStrut(6));

        // Status Row
        JPanel statusRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 6, 0));
        statusRow.setBackground(WHITE);
        statusRow.setAlignmentX(Component.LEFT_ALIGNMENT); // Left align
        statusRow.setMaximumSize(new Dimension(Integer.MAX_VALUE, 25));
        ...
        statusLbl.setForeground(new Color(60, 160, 80));

        statusRow.add(dot);
        statusRow.add(statusLbl);
        textSection.add(statusRow);
        textSection.add(Box.createVerticalStrut(8));

        // Hashtags
        JLabel tagsLbl = new JLabel(tags);
        tagsLbl.setFont(new Font("SansSerif", Font.BOLD, 12));
        tagsLbl.setForeground(ACCENT);
        tagsLbl.setAlignmentX(Component.LEFT_ALIGNMENT); // Left align
        textSection.add(tagsLbl);

        textSection.add(Box.createVerticalStrut(15));
        card.add(textSection);

        // 3. REQUEST BUTTON (Now Left Aligned to match text)
        JPanel btnWrapper = new JPanel(new FlowLayout(FlowLayout.LEFT, 15, 0));
        ...
        reqBtn.setPreferredSize(new Dimension(180, 36));
        reqBtn.addActionListener(e -> new RequestItemPage(studentName, name, owner, status, tags));

        btnWrapper.add(reqBtn);
        card.add(btnWrapper);
